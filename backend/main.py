...
name: main
layer: Presentation
depends: [config, db, models, parser, bot]
responsibility: FastAPI application assembly — CORS, lifespan, endpoints (/health, /api/parse, GET /api/articles/{id})
contract: All endpoints return valid JSON; health always responds 200; parse delegates to parser module; lifespan manages db init/close and bot polling lifecycle
</MODULE_CONTRACT>

<LINKS>
- config: uses settings for CORS origins, db_path
- db: init_db() in lifespan; get_article_by_id, get_article_by_url, save_article in endpoints
- models: ParseRequest, Article
