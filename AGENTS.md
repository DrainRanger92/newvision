...
Перевод открывается по кнопке-иконке (🌐) слева от абзаца — icon-trigger вместо свайпа.
...
## Current State (09.07.2026)

### Production

- **Cloud Run revision**: `00030` (latest productive)
- **Region**: `europe-west1`
- **minScale**: `1` (always-warm instance, webhook never dropped)
- **Webhook URL**: `https://newvision-backend-280105212513.europe-west1.run.app/webhook/telegram`
- **delete_webhook**: removed from lifespan shutdown (PR #96) — webhook persists across restarts
- **Port 8080** consistently used everywhere: Dockerfile `EXPOSE 8080`, cloudbuild `--port=8080`, docker-compose `8080:8080` (PR #98)
- **Secrets in GCP Secret Manager**: `deepseek-api-key`, `bot-token`, `webhook-secret` — all 3 configured via `--set-secrets` in `cloudbuild.yaml`

### Completed Features (P3 — M5 Curtain UX)

- **Icon-trigger** (🌐 globe button) instead of swipe gesture (PR #95) — resolves iOS swipe-to-scroll conflict
- **Batch-prefetch** translations via `POST /api/translate/batch` — `useBatchPrefetch` hook with `AbortController` pre-warms cache on article load
- **Code blocks are NOT translated** — `BlockType.code` and `BlockType.image` excluded from translation pipeline

### Infrastructure

- `docker-compose.yml`: backend `8080:8080`, frontend `5173:5173`, healthcheck on port 8080 (PR #98)
- `cloudbuild.yaml`: `--port=8080`, `--min-instances=1`, `--set-secrets` for all 3 secrets
...
| # | Title | Milestone |
|---|-------|-----------|
| #74 | Persistent DB (PostgreSQL) | M8 |
| #94 | AbortController in useBatchPrefetch | M5 |
| #97 | Icon visibility on dark theme | M6 |
...
| Require approval of most recent push | ✅ | После любого пуша approve сбрасывается — нужно переаппрувить |
| Dismiss stale approvals | ✅ | Новый коммит сбрасывает старый approve |
| Require conversation resolution | ✅ | Все PR-комментарии должны быть resolved |
...
│   │   ├── components/
│   │   │   ├── ArticleRenderer.tsx
│   │   │   ├── CurtainBlock.tsx      # core: icon-trigger translation
│   │   │   ├── CodeBlock.tsx
│   │   │   └── Header.tsx
│   │   ├── hooks/
│   │   │   ├── useCurtain.ts
...
- [x] **M5** Curtain UX: icon-trigger (🌐), batch-prefetch, 60fps, code blocks excluded ✅ (9f86a94, #95)
- **M6** Telegram theme + polish: light/dark, icon visibility on dark theme (#97), inline-code preserved
- **M7** Deploy: GCP Cloud Run + HTTPS, smoke на реальном устройстве
- **M8** Persistent DB: PostgreSQL, migration from SQLite (#74)
...
| M5 Curtain UX | Full race (2+3) | $5–15 | Icon-trigger, batch-prefetch, animation — DONE ✅ |
| M6 Theme + Polish | Single builder | $0.30–1 | CSS variables, dark theme icon fix (#97) |
| M7 Deploy | Single builder | $0.30–1 | GCP Cloud Run, HTTPS, smoke on device |
| M8 Persistent DB | Single builder | $0.30–1 | PostgreSQL migration (#74) |
...
> **M5 — завершён.** Следующие milestone (M6-M8) используют economy-режим (single builder, $0.30–1).
...
- [x] Deploy platform: GCP Cloud Run (europe-west1), not VPS/Hetzner
- [x] Max budget per milestone: $5 for economy, $15 for full race. Acceptable? (Mark: confirm)
- [x] Should LogCraft run even when all gates pass? (recommended: yes, for audit trail)
...
- **M5**: Curtain UX ✅ (PR #95): icon-trigger replaces swipe, batch-prefetch via /api/translate/batch, AbortController, code blocks excluded
- **Production**: Cloud Run rev 00030, minScale=1, webhook persists (no delete on shutdown)
