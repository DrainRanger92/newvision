...
| DB | SQLite (cache) |
| Anim | Framer Motion |
Translation | LLM через DeepSeek API (OpenAI-compatible) | EN→RU в проде
| LLM provider | deepseek/* (deepseek-chat, deepseek-v4-pro, deepseek-v4-flash) |

## GRACE Semantic Markup Convention

This project uses **GRACE Framework** (Generative Robust AI Code Engineering). All `backend/*.py` files MUST contain `MODULE_CONTRACT` + `LINKS` docstrings. See `docs/grace/MODULE_MAP.xml` for the knowledge graph. Semantic log anchors: `[ModuleName]` prefix.

GRACE compliance rules:
1. Each `.py` file has `<MODULE_CONTRACT>` + `<LINKS>` docstring
2. Logs use `[ModuleName]` anchor prefix
3. Public functions have docstrings (PEP 257)
4. Layer boundaries: Presentation → Application → Domain → Data
5. `MODULE_MAP.xml` updated synchronously with any module change

## Available Models (OpenCode Go — subscription)

| Роль цикла | Модель | Зачем |
|---|---|---|
| **Architect #1** | `opencode-go/qwen3.7-max` | Сильный reasoning, baseline-план |
| **Architect #2** | `opencode-go/kimi-k2.6` | Альтернативный взгляд, находит другие риски |
| **Architect #3** | `opencode-go/qwen3.7-plus` | Бюджетный вариант, быстрый |
| **Plan Validator** | `opencode-go/deepseek-v4-pro` | Выбирает лучший план из трёх (не участвует в architect-пуле) |
| **Build #1** | `opencode-go/deepseek-v4-flash` | Быстрый coding, M2 winner |
| **Build #2** | `opencode-go/qwen3.6-plus` | Альтернативный coding подход |
| **Build #3** | `opencode-go/glm-5.1` | Сильный baseline (если не зависнет в env) |
| **Build Validator** | `opencode-go/deepseek-v4-pro` | Глубокий ревью кода, data integrity check |
| **Translate (runtime)** | `opencode-go/mimo-v2.5-pro` | EN→RU в проде |

> ⚠️ Только `opencode-go/*` модели. Plan validator ≠ все architects. Build validator ≠ все builders.
